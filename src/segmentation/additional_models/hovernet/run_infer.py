"""run_infer.py - HoverNet Inference Lite

A lightweight, standalone version for running HoverNet inference on image tiles.
...
Command mode is `tile` to enter tile inference mode:
    tile  run the inference on tiles

Use `run_infer.py <command> --help` to show options and usage.
"""
...
if __name__ == "__main__":
    ...
    args = docopt(
        ...
        help=False,
        options_first=True,
        version="HoVer-Net Pytorch Inference Lite v1.0",
    )
    ...
    if sub_cmd not in sub_cli_dict:
        print(f"Unknown command: {sub_cmd}")
        print("This lite version only supports 'tile' mode.")
        print(__doc__)
        exit(1)
