...
4. Optional converted full-slice GT evaluation arrays under
   ...
   These arrays are not generated by the public setup script yet, so
   `gt_predict_eval.py` is documented as an important validation entry point but
   is not a fully reproducible public step.
