...
| `load_wsi_and_geojson_data_from_paths` | Loads a TIFF WSI crop and matching GeoJSON from explicit paths. | No Python source callers found after public notebook cleanup. |
| `load_image_and_annotations` | Deprecated loader tied to a removed legacy layout. | No Python source callers found after public notebook cleanup. |
...
| `round_polygon_coords` | Rounds polygon coordinates, simplifies geometry, and returns a valid polygon when possible. | `src/segmentation/additional_models/adaptive_threshold/adaptive_threshold_model.py:13`, `src/segmentation/additional_models/adaptive_threshold/adaptive_threshold_model.py:211`. |
